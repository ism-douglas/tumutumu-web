...
class AssignmentsController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				assignments.academic_term LIKE ?
			)";
			$search_params = array(
	...
	function view($rec_id = null, $value = null){
		...
		if($record){
			$page_title = $this->view->page_title = "View  Assignments";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
...
$db->rawQuery("UPDATE assignments SET uploaded_by = $USER_ID  WHERE id='$rec_id'");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","subject","views","uploaded_by","document","date_created","date_updated","remarks","level","academic_year","academic_term");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'subject' => 'required',
